...
function macroexpand(form)
  ...
  elseif atom63(form) then
    ...
    elseif (x == "%function") then
      local _g3 = form[1]
      local args = form[2]
      local _g126 = sub(form, 2)
      add(environment, {})
      local _g128 = (function ()
        local _g130 = 0
        local _g129 = args
        while (_g130 < length(_g129)) do
          local _g127 = _g129[(_g130 + 1)]
...
function sub(x, from, upto)
  local _g140 = (from or 0)
  if string63(x) then
    return((string.sub)(x, (_g140 + 1), upto))
  else
    local l = (function ()
      local i = _g140
      local j = 0
...
function join(l1, l2)
  ...
  elseif nil63(l2) then
    ...
    local _g144 = nil
    local _g143 = l2
    for _g144 in next, _g143 do
      if (not number63(_g144)) then
        local v = _g143[_g144]
        l[_g144] = v
      end
    end
...
function cat(...)
  local xs = unstash({...})
  local _g162 = sub(xs, 0)
  if empty63(_g162) then
    return("")
  else
    return(reduce(function (a, b)
      return((a .. b))
...
function apply(f, args)
  local _g172 = stash(args)
  return(f(unpack(_g172)))
end
id_count = 0
function make_id()
  id_count = (id_count + 1)
...
setenv("define-module", {macro = function (spec, ...)
  ...
  local imp = _g180.import
  local exp = _g180.export
  map(load_module, imp)
  imports = imp
  exports = {}
  local _g182 = 0
...
modules.main = {}
modules.reader = {}
modules.reader["define-reader"] = {macro = function (_g183, ...)
  local char = _g183[1]
  local stream = _g183[2]
  local body = unstash({...})
  local _g184 = sub(body, 0)
  return({"set", {"get", "read-table", char}, join({"fn", {stream}}, _g184)})
end}
modules.lib = {}
modules.lib["define-local"] = {macro = function (name, x, ...)
  ...
  if (not empty63(_g185)) then
    ...
    local args = _g186[1]
    local _g187 = _g186[2]
    return(join({"%local-function", name, args}, _g187))
  else
    return({"%local", name, x})
  end
end}
modules.lib["let-symbol"] = {macro = function (expansions, ...)
  ...
  add(environment, {})
  local _g189 = (function ()
    map(function (_g190)
      local name = _g190[1]
      local exp = _g190[2]
      return(macroexpand({"define-symbol", name, exp}))
    end, pairwise(expansions))
    return(join({"do"}, macroexpand(_g188)))
  end)()
  drop(environment)
  return(_g189)
end}
...
modules.lib.fn = {macro = function (args, ...)
  ...
  local _g198 = bind_arguments(args, _g197)
  local args = _g198[1]
  local _g199 = _g198[2]
...
modules.lib["with-bindings"] = {macro = function (_g206, ...)
  local names = _g206[1]
  local body = unstash({...})
  local _g207 = sub(body, 0)
  local x = make_id()
  return(join({"with-frame", {"across", {names, x}, {"setenv*", x, "pending"}}}, _g207))
end}
...
modules.lib.let = {macro = function (bindings, ...)
  ...
  map(function (_g211)
    ...
    local rh = _g211[2]
    local _g213 = 0
    local _g212 = bind(lh, rh)
    while (_g213 < length(_g212)) do
      local _g214 = _g212[(_g213 + 1)]
      local id = _g214[1]
      local val = _g214[2]
      if bound63(id) then
...
modules.lib["set-of"] = {macro = function (...)
  ...
  local l = {}
  local _g216 = 0
  local _g215 = elements
  while (_g216 < length(_g215)) do
    local e = _g215[(_g216 + 1)]
    l[e] = true
    _g216 = (_g216 + 1)
  end
  return(join({"table"}, l))
end}
modules.lib.across = {macro = function (_g217, ...)
  local l = _g217[1]
  local v = _g217[2]
  local i = _g217[3]
  local start = _g217[4]
...
modules.lib.each = {macro = function (_g220, ...)
  local t = _g220[1]
  local k = _g220[2]
  local v = _g220[3]
  local body = unstash({...})
...
modules.compiler = {}
modules.compiler["return"] = {special = function (_g223)
  local x = _g223[1]
  local x = (function ()
    if nil63(x) then
      return("return")
...
modules.compiler["define-module"] = {macro = function (spec, ...)
  ...
  local imp = _g224.import
  local exp = _g224.export
  map(load_module, imp)
  imports = imp
  exports = {}
  local _g226 = 0
...
modules.compiler["%global-function"] = {special = function (_g229)
  local name = _g229[1]
  local args = _g229[2]
...
modules.compiler["%local"] = {special = function (_g230)
  local name = _g230[1]
  local value = _g230[2]
...
modules.compiler["while"] = {special = function (_g232)
...
modules.compiler["error"] = {special = function (_g234)
  local x = _g234[1]
  local e = (function ()
    if (target == "js") then
...
modules.compiler["not"] = {special = function (_g238)
  local x = _g238[1]
  local x = compile(x)
  local open = (function ()
    if (target == "js") then
...
modules.compiler["%local-function"] = {special = function (_g239)
  local name = _g239[1]
  local args = _g239[2]
  local body = sub(_g239, 2)
  return(compile_function(args, body, {_stash = true, name = name, prefix = "local "}))
end, stmt = true, tr = true}
modules.compiler["set"] = {special = function (_g240)
  local lh = _g240[1]
  local rh = _g240[2]
  if nil63(rh) then
...
modules.compiler["%function"] = {special = function (_g244)
  local args = _g244[1]
  local body = sub(_g244, 1)
  return(compile_function(args, body))
end}
modules.compiler["get"] = {special = function (_g245)
  local t = _g245[1]
  local k = _g245[2]
  local t = compile(t)
  local k1 = compile(k)
