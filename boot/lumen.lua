...
(function ()
  ...
  local function precedence(op, args)
    if not (unary63(op, args)) then
      local _g154 = infix
      local i = 0
  ...
  local function compile_infix(_g157)
    local op = _g157[1]
    local args = sub(_g157, 1)
    local a = args[1]
    local b = args[2]
    local op1 = getop(op)
  ...
  local function compile_function(args, body, ...)
    local _g158 = unstash({...})
    local name = _g158.name
    local prefix = _g158.prefix
    local _g163
    if name then
      _g163 = compile(name)
    else
    ...
    local id = _g163
    local _g159 = (prefix or "")
    local _g160 = compile_args(args)
    indent_level = (indent_level + 1)
    local _g162 = compile(body, {_stash = true, stmt = true})
    indent_level = (indent_level - 1)
    local _g161 = _g162
    ...
    if (target == "js") then
      return(("function " .. (id .. (_g160 .. (" {\n" .. (_g161 .. (ind .. ("}" .. tr))))))))
    else
      return((_g159 .. ("function " .. (id .. (_g160 .. ("\n" .. (_g161 .. (ind .. tr))))))))
    end
  end
  ...
  compile = function (form, ...)
    ...
    if nil63(form) then
      ...
      if special_form63(form) then
        ...
        local _g168
        if atom63(form) then
          _g168 = compile_atom(form)
        else
          local _g169
          if infix63(hd(form)) then
            _g169 = compile_infix(form)
          else
            _g169 = compile_call(form)
          end
          _g168 = _g169
        end
        local _g166 = _g168
  ...
  local function lower_if(args, hoist, stmt63, tail63)
    ...
    if (stmt63 or tail63) then
      local _g175
      if _g173 then
        _g175 = {lower_body({_g173}, tail63)}
      end
      return(add(hoist, join({"%if", lower(cond, hoist), lower_body({_g172}, tail63)}, _g175)))
    else
      local e = make_id()
      add(hoist, {"%local", e})
      local _g174
      if _g173 then
        _g174 = {lower({"set", e, _g173})}
      end
      add(hoist, join({"%if", lower(cond, hoist), lower({"set", e, _g172})}, _g174))
      return(e)
    end
  end
  local function lower_short(x, args, hoist)
    ...
    if some63(hoist1) then
      ...
      return(lower({"do", {"%local", id, a}, _g176}, hoist))
    else
  ...
  local function encapsulate(body)
    local _g179 = map(process, body)
    local epilog = map(process, exported())
    return({{"%function", {}, join({"do"}, join(_g179, epilog))}})
  end
  ...
  local function eval(form)
    ...
    target = "lua"
    local _g183 = {join({"%function", {}}, join(prologue(), {form}))}
    local compiled = compile(process(_g183))
    target = previous
    return(run(compiled))
  end
  local _g184 = {}
  nexus.compiler = _g184
  _g184["compile-function"] = compile_function
  _g184.compile = compile
  _g184["open-module"] = open_module
  _g184["load-module"] = load_module
  _g184["in-module"] = in_module
  _g184["compile-module"] = compile_module
  _g184.eval = eval
  _g184.infix = infix
  _g184["unary?"] = unary63
  _g184.precedence = precedence
  _g184.getop = getop
  _g184["infix?"] = infix63
  _g184["compile-args"] = compile_args
  _g184["compile-atom"] = compile_atom
  _g184.terminator = terminator
  _g184["compile-special"] = compile_special
  _g184["compile-call"] = compile_call
  _g184["compile-infix"] = compile_infix
  _g184["can-return?"] = can_return63
  _g184.lower = lower
  _g184["lower-statement"] = lower_statement
  _g184["lower-body"] = lower_body
  _g184["lower-do"] = lower_do
  _g184["lower-if"] = lower_if
  _g184["lower-short"] = lower_short
  _g184["lower-try"] = lower_try
  _g184["lower-while"] = lower_while
  _g184["lower-for"] = lower_for
  _g184["lower-function"] = lower_function
  _g184["lower-definition"] = lower_definition
  _g184["lower-call"] = lower_call
  _g184["lower-infix?"] = lower_infix63
  _g184["lower-infix"] = lower_infix
  _g184["lower-special"] = lower_special
  _g184.process = process
  _g184["module-path"] = module_path
  _g184.encapsulate = encapsulate
  _g184["compile-file"] = compile_file
  _g184.run = run
  _g184["compiling?"] = compiling63
  _g184["compiler-output"] = compiler_output
  _g184["%compile-module"] = _37compile_module
  _g184.prologue = prologue
end)();
(function ()
  local _g185 = nexus.runtime
  local nil63 = _g185["nil?"]
  local is63 = _g185["is?"]
  local length = _g185.length
  local none63 = _g185["none?"]
  local some63 = _g185["some?"]
  local in63 = _g185["in?"]
  local hd = _g185.hd
  local string63 = _g185["string?"]
  local number63 = _g185["number?"]
  local boolean63 = _g185["boolean?"]
  local function63 = _g185["function?"]
  local composite63 = _g185["composite?"]
  local atom63 = _g185["atom?"]
  local table63 = _g185["table?"]
  local list63 = _g185["list?"]
  local substring = _g185.substring
  local sublist = _g185.sublist
  local sub = _g185.sub
  local inner = _g185.inner
  local tl = _g185.tl
  local char = _g185.char
  local code = _g185.code
  local string_literal63 = _g185["string-literal?"]
  local id_literal63 = _g185["id-literal?"]
  local add = _g185.add
  local drop = _g185.drop
  local last = _g185.last
  local reverse = _g185.reverse
  local join = _g185.join
  local reduce = _g185.reduce
  local keep = _g185.keep
  local find = _g185.find
  local pairwise = _g185.pairwise
  local iterate = _g185.iterate
  local replicate = _g185.replicate
  local splice = _g185.splice
  local map = _g185.map
  local keys63 = _g185["keys?"]
  local empty63 = _g185["empty?"]
  local stash = _g185.stash
  local unstash = _g185.unstash
  local extend = _g185.extend
  local exclude = _g185.exclude
  local search = _g185.search
  local split = _g185.split
  local cat = _g185.cat
  local _43 = _g185["+"]
  local _ = _g185["-"]
  local _42 = _g185["*"]
  ...
  local _6261 = _g185[">="]
  local _6061 = _g185["<="]
  local read_file = _g185["read-file"]
  local write_file = _g185["write-file"]
  local write = _g185.write
  local exit = _g185.exit
  local parse_number = _g185["parse-number"]
  local to_string = _g185["to-string"]
  local apply = _g185.apply
  local make_id = _g185["make-id"]
  local _37message_handler = _g185["%message-handler"]
  local toplevel63 = _g185["toplevel?"]
  local module_key = _g185["module-key"]
  local module = _g185.module
  local setenv = _g185.setenv
  local _g186 = nexus.utilities
  local getenv = _g186.getenv
  local macro_function = _g186["macro-function"]
  local macro63 = _g186["macro?"]
  local special63 = _g186["special?"]
  local special_form63 = _g186["special-form?"]
  local statement63 = _g186["statement?"]
  local symbol_expansion = _g186["symbol-expansion"]
  local symbol63 = _g186["symbol?"]
  local variable63 = _g186["variable?"]
  local bound63 = _g186["bound?"]
  local toplevel63 = _g186["toplevel?"]
  local quoted = _g186.quoted
  local stash42 = _g186["stash*"]
  local bind = _g186.bind
  local bind42 = _g186["bind*"]
  local quasiexpand = _g186.quasiexpand
  local macroexpand = _g186.macroexpand
  local indentation = _g186.indentation
  local reserved63 = _g186["reserved?"]
  local valid_id63 = _g186["valid-id?"]
  local to_id = _g186["to-id"]
  local imported = _g186.imported
  local exported = _g186.exported
  local mapo = _g186.mapo
  local quote_environment = _g186["quote-environment"]
  local quote_modules = _g186["quote-modules"]
  local initial_environment = _g186["initial-environment"]
  local _g189 = nexus.compiler
  local compile_function = _g189["compile-function"]
  local compile = _g189.compile
  local open_module = _g189["open-module"]
  local load_module = _g189["load-module"]
  local in_module = _g189["in-module"]
  local compile_module = _g189["compile-module"]
  local eval = _g189.eval
  local lower = _g189.lower
end)();
(function ()
  local _g347 = nexus.runtime
  local nil63 = _g347["nil?"]
  local is63 = _g347["is?"]
  local length = _g347.length
  local none63 = _g347["none?"]
  local some63 = _g347["some?"]
  local in63 = _g347["in?"]
  local hd = _g347.hd
  local string63 = _g347["string?"]
  local number63 = _g347["number?"]
  local boolean63 = _g347["boolean?"]
  local function63 = _g347["function?"]
  local composite63 = _g347["composite?"]
  local atom63 = _g347["atom?"]
  local table63 = _g347["table?"]
  local list63 = _g347["list?"]
  local substring = _g347.substring
  local sublist = _g347.sublist
  local sub = _g347.sub
  local inner = _g347.inner
  local tl = _g347.tl
  local char = _g347.char
  local code = _g347.code
  local string_literal63 = _g347["string-literal?"]
  local id_literal63 = _g347["id-literal?"]
  local add = _g347.add
  local drop = _g347.drop
  local last = _g347.last
  local reverse = _g347.reverse
  local join = _g347.join
  local reduce = _g347.reduce
  local keep = _g347.keep
  local find = _g347.find
  local pairwise = _g347.pairwise
  local iterate = _g347.iterate
  local replicate = _g347.replicate
  local splice = _g347.splice
  local map = _g347.map
  local keys63 = _g347["keys?"]
  local empty63 = _g347["empty?"]
  local stash = _g347.stash
  local unstash = _g347.unstash
  local extend = _g347.extend
  local exclude = _g347.exclude
  local search = _g347.search
  local split = _g347.split
  local cat = _g347.cat
  local _43 = _g347["+"]
  local _ = _g347["-"]
  local _42 = _g347["*"]
  ...
  local _6261 = _g347[">="]
  local _6061 = _g347["<="]
  local read_file = _g347["read-file"]
  local write_file = _g347["write-file"]
  local write = _g347.write
  local exit = _g347.exit
  local parse_number = _g347["parse-number"]
  local to_string = _g347["to-string"]
  local apply = _g347.apply
  local make_id = _g347["make-id"]
  local _37message_handler = _g347["%message-handler"]
  local toplevel63 = _g347["toplevel?"]
  local module_key = _g347["module-key"]
  local module = _g347.module
  local setenv = _g347.setenv
  local _g348 = nexus.utilities
  local getenv = _g348.getenv
  local macro_function = _g348["macro-function"]
  local macro63 = _g348["macro?"]
  local special63 = _g348["special?"]
  local special_form63 = _g348["special-form?"]
  local statement63 = _g348["statement?"]
  local symbol_expansion = _g348["symbol-expansion"]
  local symbol63 = _g348["symbol?"]
  local variable63 = _g348["variable?"]
  local bound63 = _g348["bound?"]
  local toplevel63 = _g348["toplevel?"]
  local quoted = _g348.quoted
  local stash42 = _g348["stash*"]
  local bind = _g348.bind
  local bind42 = _g348["bind*"]
  local quasiexpand = _g348.quasiexpand
  local macroexpand = _g348.macroexpand
  local indentation = _g348.indentation
  local reserved63 = _g348["reserved?"]
  local valid_id63 = _g348["valid-id?"]
  local to_id = _g348["to-id"]
  local imported = _g348.imported
  local exported = _g348.exported
  local mapo = _g348.mapo
  local quote_environment = _g348["quote-environment"]
  local quote_modules = _g348["quote-modules"]
  local initial_environment = _g348["initial-environment"]
  local _g351 = nexus.compiler
  local compile_function = _g351["compile-function"]
  local compile = _g351.compile
  local open_module = _g351["open-module"]
  local load_module = _g351["load-module"]
  local in_module = _g351["in-module"]
  local compile_module = _g351["compile-module"]
  local eval = _g351.eval
  local lower = _g351.lower
  target = "lua"
end)();
(function ()
  ...
  local statement63 = _g618["statement?"]
  ...
  local lower = _g621.lower
  modules = {main = {import = {"runtime", "special", "core", "reader", "compiler"}, export = {save = {macro = function (...)
    local specs = unstash({...})
    local _g634 = sub(specs, 0)
    map(compile_module, _g634)
    return(nil)
  end}}}, special = {import = {"runtime", "utilities", "special", "core", "compiler"}, export = {["do"] = {stmt = true, tr = true, export = true, special = function (...)
    local forms = unstash({...})
    local str = ""
    local _g635 = forms
    local _g636 = 0
    while (_g636 < length(_g635)) do
      local x = _g635[(_g636 + 1)]
      str = (str .. compile(x, {_stash = true, stmt = true}))
      _g636 = (_g636 + 1)
    end
    return(str)
  end, foo = true}, ["%if"] = {stmt = true, tr = true, export = true, special = function (cond, cons, alt)
    local _g637 = compile(cond)
    indent_level = (indent_level + 1)
    local _g640 = compile(cons, {_stash = true, stmt = true})
    indent_level = (indent_level - 1)
    local _g638 = _g640
    local _g705
    if alt then
      indent_level = (indent_level + 1)
      local _g641 = compile(alt, {_stash = true, stmt = true})
      indent_level = (indent_level - 1)
      _g705 = _g641
    end
    local _g639 = _g705
  ...
  end, foo = true}, ["while"] = {stmt = true, tr = true, export = true, special = function (cond, form)
    local _g642 = compile(cond)
    indent_level = (indent_level + 1)
    local _g643 = compile(form, {_stash = true, stmt = true})
    indent_level = (indent_level - 1)
    local body = _g643
    local ind = indentation()
    if (target == "js") then
  ...
  end, foo = true}, ["%for"] = {stmt = true, tr = true, export = true, special = function (t, k, form)
  ...
  end, foo = true}, ["%try"] = {stmt = true, tr = true, export = true, special = function (form)
    local ind = indentation()
    indent_level = (indent_level + 1)
    local _g646 = compile(form, {_stash = true, stmt = true})
    indent_level = (indent_level - 1)
    local body = _g646
    local e = make_id()
    local hf = {"return", {"%array", false, {"get", e, "\"message\""}}}
    indent_level = (indent_level + 1)
    local _g647 = compile(hf, {_stash = true, stmt = true})
    indent_level = (indent_level - 1)
    local h = _g647
    return((ind .. ("try {\n" .. (body .. (ind .. ("}\n" .. (ind .. ("catch (" .. (e .. (") {\n" .. (h .. (ind .. "}\n"))))))))))))
  ...
  end, export = true, foo = true}, ["%global-function"] = {stmt = true, tr = true, export = true, special = function (name, args, body)
    if (target == "lua") then
  ...
  end, foo = true}, ["%local-function"] = {stmt = true, tr = true, export = true, special = function (name, args, body)
    ...
    local _g707
    if (target == "js") then
      _g707 = ("throw new " .. compile({"Error", x}))
    else
      _g707 = ("error(" .. (compile(x) .. ")"))
    end
    local e = _g707
    ...
    local _g650 = compile(_g710)
    return((indentation() .. (_g649 .. (" = " .. _g650))))
  end, export = true, foo = true}, ["get"] = {special = function (t, k)
    local _g651 = compile(t)
    local k1 = compile(k)
    if ((target == "lua") and (char(_g651, 0) == "{")) then
      _g651 = ("(" .. (_g651 .. ")"))
    end
    ...
    local _g653 = pairs
    local i = 0
    while (i < length(_g653)) do
      local _g654 = _g653[(i + 1)]
      local k = _g654[1]
      local v = _g654[2]
      if not (string63(k)) then
        error(("Illegal key: " .. to_string(k)))
      end
      local _g655 = compile(v)
      local _g714
    ...
    return({"if", {"not", cond}, join({"do"}, _g662)})
  end, export = true}, table = {macro = function (...)
    local body = unstash({...})
    return(join({"%object"}, mapo(function (_g346, x)
      return(x)
    end, body)))
    ...
    map(function (_g664)
      local lh = _g664[1]
      local rh = _g664[2]
      local _g665 = bind(lh, rh)
      local _g666 = 0
      while (_g666 < length(_g665)) do
        local _g667 = _g665[(_g666 + 1)]
        local id = _g667[1]
        local val = _g667[2]
    ...
    local imports = {}
    local imp = _g668.import
    local exp = _g668.export
    local _g669 = (imp or {})
    local _g670 = 0
    while (_g670 < length(_g669)) do
      local k = _g669[(_g670 + 1)]
      load_module(k)
    ...
    if some63(_g677) then
      local _g678 = bind42(x, _g677)
      local args = _g678[1]
      local _g679 = _g678[2]
    ...
    if some63(_g680) then
      local _g681 = bind42(x, _g680)
      local args = _g681[1]
      local _g682 = _g681[2]
  ...
  end, export = true}, ["with-bindings"] = {macro = function (_g683, ...)
    local names = _g683[1]
    local body = unstash({...})
    ...
    _g686.variable = true
    local _g685 = {"with-frame", {"each", {x}, names, _g686}}
    _g685.scope = true
    return(join(_g685, _g684))
    ...
    local _g688 = join({"do"}, macroexpand(_g687))
    drop(environment)
    return(_g688)
    ...
    local _g690 = join({"do"}, macroexpand(_g689))
    drop(environment)
    return(_g690)
    ...
    if nil63(v) then
      ...
      local i = _g719
      _g718 = {"let", {i, 0}, {"while", {"<", i, {"length", t1}}, join({"let", {k, {"at", t1, i}}}, _g696), {"inc", i}}}
    else
      local _g697 = {"target"}
      _g697.js = {"isNaN", {"parseInt", k}}
      _g697.lua = {"not", {"number?", k}}
      _g718 = {"let", {k, "nil"}, {"%for", t1, k, {"when", _g697, join({"let", {v, {"get", t1, k}}}, _g696)}}}
    end
    ...
    local _g704 = {"table"}
    _g704._scope = scope
    return({"do", {"add", "environment", _g704}, {"let", {x, join({"do"}, _g703)}, {"drop", "environment"}, x}})
  end, export = true}}}}
  environment = {{["define-module"] = {macro = function (spec, ...)
    local body = unstash({...})
    local _g720 = sub(body, 0)
    local imports = {}
    local imp = _g720.import
    local exp = _g720.export
    local _g721 = (imp or {})
    local _g722 = 0
    while (_g722 < length(_g721)) do
      local k = _g721[(_g722 + 1)]
      load_module(k)
      imports = join(imports, imported(k))
      _g722 = (_g722 + 1)
    end
...
(function ()
  ...
  local function rep(str)
    local _g727,_g728 = xpcall(function ()
      return(eval(read_from_string(str)))
    end, _37message_handler)
    local _g726 = {_g727, _g728}
    local _g1 = _g726[1]
    local x = _g726[2]
    if is63(x) then
