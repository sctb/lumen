...
(function ()
  ...
  local reserved = {["void"] = true, ["="] = true, ["in"] = true, ["finally"] = true, ["for"] = true, ["case"] = true, ["debugger"] = true, ["if"] = true, ["+"] = true, ["/"] = true, ["do"] = true, ["-"] = true, ["=="] = true, ["return"] = true, [">="] = true, ["while"] = true, ["else"] = true, ["then"] = true, [">"] = true, ["nil"] = true, ["<"] = true, ["with"] = true, ["local"] = true, ["switch"] = true, ["or"] = true, ["break"] = true, ["not"] = true, ["false"] = true, ["until"] = true, ["continue"] = true, ["end"] = true, ["true"] = true, ["repeat"] = true, ["*"] = true, ["and"] = true, ["elseif"] = true, ["var"] = true, ["typeof"] = true, ["try"] = true, ["new"] = true, ["throw"] = true, ["default"] = true, ["<="] = true, ["delete"] = true, ["function"] = true, ["catch"] = true, ["instanceof"] = true, ["this"] = true, ["%"] = true}
...
(function ()
  ...
  local function op_delims(parent, p)
    if p > precedence(parent) then
      return({"(", ")"})
    else
      return({"", ""})
    end
  ...
  local function compile_infix(form)
    ...
    local a = _g157[1]
    local b = _g157[2]
    local _g158 = op_delims(form, precedence(a))
    local ao = _g158[1]
    local ac = _g158[2]
    local _g159 = op_delims(form, precedence(b) + 1)
    local bo = _g159[1]
    local bc = _g159[2]
  ...
  compile = function (form, ...)
    ...
    if nil63(form) then
      ...
      if special_form63(form) then
        ...
        local _g173
        if atom63(form) then
          _g173 = compile_atom(form)
        else
          local _g174
          if infix63(hd(form)) then
            _g174 = compile_infix(form)
          else
            _g174 = compile_call(form)
          end
          _g173 = _g174
        end
        local _g171 = _g173
  ...
  local function lower_if(args, hoist, stmt63, tail63)
    ...
    if stmt63 or tail63 then
      local _g180
      if _g178 then
        _g180 = {lower_body({_g178}, tail63)}
      end
      return(add(hoist, join({"%if", lower(cond, hoist), lower_body({_g177}, tail63)}, _g180)))
    else
      local e = make_id()
      add(hoist, {"%local", e})
      local _g179
      if _g178 then
        _g179 = {lower({"set", e, _g178})}
      end
      add(hoist, join({"%if", lower(cond, hoist), lower({"set", e, _g177})}, _g179))
      return(e)
    end
  end
  local function lower_short(x, args, hoist)
    ...
    if some63(hoist1) then
      ...
      return(lower({"do", {"%local", id, a}, _g181}, hoist))
    else
  ...
  local function encapsulate(body)
    local _g184 = map(process, body)
    local epilog = map(process, exported())
    return({{"%function", {}, join({"do"}, join(_g184, epilog))}})
  end
  ...
  local function eval(form)
    ...
    target = "lua"
    local _g188 = {join({"%function", {}}, join(prologue(), {form}))}
    local compiled = compile(process(_g188))
    target = previous
    return(run(compiled))
  end
  local _g189 = {}
  nexus.compiler = _g189
  _g189["load-module"] = load_module
  _g189.compile = compile
  _g189["lower-statement"] = lower_statement
  _g189["lower-infix?"] = lower_infix63
  _g189["lower-for"] = lower_for
  _g189["compile-args"] = compile_args
  _g189["lower-infix"] = lower_infix
  _g189["lower-function"] = lower_function
  _g189.infix = infix
  _g189["op-delims"] = op_delims
  _g189["lower-body"] = lower_body
  _g189["%compile-module"] = _37compile_module
  _g189["compile-function"] = compile_function
  _g189["infix?"] = infix63
  _g189["in-module"] = in_module
  _g189["unary?"] = unary63
  _g189["compile-infix"] = compile_infix
  _g189.terminator = terminator
  _g189["lower-try"] = lower_try
  _g189["can-return?"] = can_return63
  _g189["lower-while"] = lower_while
  _g189["compiler-output"] = compiler_output
  _g189["compile-call"] = compile_call
  _g189.run = run
  _g189.process = process
  _g189.eval = eval
  _g189["lower-definition"] = lower_definition
  _g189.getop = getop
  _g189.lower = lower
  _g189.precedence = precedence
  _g189["compile-module"] = compile_module
  _g189.prologue = prologue
  _g189["compiling?"] = compiling63
  _g189["compile-file"] = compile_file
  _g189["compile-special"] = compile_special
  _g189.encapsulate = encapsulate
  _g189["module-path"] = module_path
  _g189["lower-short"] = lower_short
  _g189["open-module"] = open_module
  _g189["lower-if"] = lower_if
  _g189["lower-special"] = lower_special
  _g189["compile-atom"] = compile_atom
  _g189["lower-call"] = lower_call
  _g189["lower-do"] = lower_do
end)();
(function ()
  local _g190 = nexus.runtime
  local replicate = _g190.replicate
  local extend = _g190.extend
  local some63 = _g190["some?"]
  local to_string = _g190["to-string"]
  local _62 = _g190[">"]
  local _61 = _g190["="]
  local _60 = _g190["<"]
  local parse_number = _g190["parse-number"]
  local iterate = _g190.iterate
  local split = _g190.split
  local exclude = _g190.exclude
  local sub = _g190.sub
  local substring = _g190.substring
  local splice = _g190.splice
  local composite63 = _g190["composite?"]
  local reduce = _g190.reduce
  local table63 = _g190["table?"]
  local reverse = _g190.reverse
  local apply = _g190.apply
  local join = _g190.join
  local module = _g190.module
  local code = _g190.code
  local boolean63 = _g190["boolean?"]
  local _ = _g190["-"]
  local inner = _g190.inner
  local _42 = _g190["*"]
  local length = _g190.length
  local _37 = _g190["%"]
  local empty63 = _g190["empty?"]
  local search = _g190.search
  local pairwise = _g190.pairwise
  local keep = _g190.keep
  local is63 = _g190["is?"]
  local map = _g190.map
  local write = _g190.write
  local _6061 = _g190["<="]
  local _6261 = _g190[">="]
  local id_literal63 = _g190["id-literal?"]
  local write_file = _g190["write-file"]
  local _37message_handler = _g190["%message-handler"]
  local string63 = _g190["string?"]
  local _43 = _g190["+"]
  local find = _g190.find
  local sublist = _g190.sublist
  local string_literal63 = _g190["string-literal?"]
  local keys63 = _g190["keys?"]
  local setenv = _g190.setenv
  local read_file = _g190["read-file"]
  local add = _g190.add
  local toplevel63 = _g190["toplevel?"]
  local stash = _g190.stash
  local make_id = _g190["make-id"]
  local none63 = _g190["none?"]
  local atom63 = _g190["atom?"]
  local exit = _g190.exit
  local tl = _g190.tl
  local module_key = _g190["module-key"]
  local nil63 = _g190["nil?"]
  local number63 = _g190["number?"]
  local _47 = _g190["/"]
  local cat = _g190.cat
  local unstash = _g190.unstash
  local last = _g190.last
  local list63 = _g190["list?"]
  local hd = _g190.hd
  local char = _g190.char
  local drop = _g190.drop
  local function63 = _g190["function?"]
  local in63 = _g190["in?"]
  local _g191 = nexus.utilities
  local special_form63 = _g191["special-form?"]
  local toplevel63 = _g191["toplevel?"]
  local variable63 = _g191["variable?"]
  local mapo = _g191.mapo
  local imported = _g191.imported
  local quoted = _g191.quoted
  local bound63 = _g191["bound?"]
  local statement63 = _g191["statement?"]
  local bind42 = _g191["bind*"]
  local quasiexpand = _g191.quasiexpand
  local symbol63 = _g191["symbol?"]
  local exported = _g191.exported
  local getenv = _g191.getenv
  local stash42 = _g191["stash*"]
  local initial_environment = _g191["initial-environment"]
  local quote_modules = _g191["quote-modules"]
  local quote_environment = _g191["quote-environment"]
  local special63 = _g191["special?"]
  local reserved63 = _g191["reserved?"]
  local symbol_expansion = _g191["symbol-expansion"]
  local macro63 = _g191["macro?"]
  local indentation = _g191.indentation
  local valid_id63 = _g191["valid-id?"]
  local macro_function = _g191["macro-function"]
  local to_id = _g191["to-id"]
  local bind = _g191.bind
  local macroexpand = _g191.macroexpand
  local _g194 = nexus.compiler
  local load_module = _g194["load-module"]
  local compile = _g194.compile
  local compile_function = _g194["compile-function"]
  local in_module = _g194["in-module"]
  local eval = _g194.eval
  local lower = _g194.lower
  local compile_module = _g194["compile-module"]
  local open_module = _g194["open-module"]
end)();
(function ()
  ...
  local to_string = _g352["to-string"]
  local _62 = _g352[">"]
  local _61 = _g352["="]
  local _60 = _g352["<"]
  local parse_number = _g352["parse-number"]
...
(function ()
  ...
  local to_string = _g622["to-string"]
  local _62 = _g622[">"]
  local _61 = _g622["="]
  local _60 = _g622["<"]
  local parse_number = _g622["parse-number"]
  ...
  local lower = _g626.lower
  local compile_module = _g626["compile-module"]
  local open_module = _g626["open-module"]
  modules = {special = {export = {["get"] = {special = function (t, k)
    local _g639 = compile(t)
    local k1 = compile(k)
    if target == "lua" and char(_g639, 0) == "{" then
      _g639 = "(" .. (_g639 .. ")")
    end
    if string_literal63(k) and valid_id63(inner(k)) then
      return(_g639 .. ("." .. inner(k)))
    else
      return(_g639 .. ("[" .. (k1 .. "]")))
    end
    ...
    while i < length(_g640) do
      local _g641 = _g640[i + 1]
      local k = _g641[1]
      local v = _g641[2]
      if not string63(k) then
        error("Illegal key: " .. to_string(k))
      end
      local _g642 = compile(v)
      local _g711
  ...
  end, export = true, foo = true}, ["%for"] = {special = function (t, k, form)
  ...
  end, tr = true, foo = true, stmt = true, export = true}, ["%local-function"] = {special = function (name, args, body)
    local x = compile_function(args, body, {_stash = true, name = name, prefix = "local "})
    return(indentation() .. x)
  end, tr = true, foo = true, stmt = true, export = true}, ["do"] = {special = function (...)
    local forms = unstash({...})
    local str = ""
    local _g646 = forms
    local _g647 = 0
    while _g647 < length(_g646) do
      local x = _g646[_g647 + 1]
      str = str .. compile(x, {_stash = true, stmt = true})
      _g647 = _g647 + 1
    end
    return(str)
  end, tr = true, foo = true, stmt = true, export = true}, ["set"] = {stmt = true, export = true, special = function (lh, rh)
    local _g648 = compile(lh)
    local _g715
    if nil63(rh) then
      _g715 = "nil"
    else
    ...
    local _g649 = compile(_g715)
    return(indentation() .. (_g648 .. (" = " .. _g649)))
  end, foo = true}, ["%if"] = {special = function (cond, cons, alt)
    local _g650 = compile(cond)
    indent_level = indent_level + 1
    local _g653 = compile(cons, {_stash = true, stmt = true})
    indent_level = indent_level - 1
    local _g651 = _g653
    local _g716
    if alt then
      indent_level = indent_level + 1
      local _g654 = compile(alt, {_stash = true, stmt = true})
      indent_level = indent_level - 1
      _g716 = _g654
    end
    local _g652 = _g716
    ...
    local str = ""
    if target == "js" then
      str = str .. (ind .. ("if (" .. (_g650 .. (") {\n" .. (_g651 .. (ind .. "}"))))))
    else
      str = str .. (ind .. ("if " .. (_g650 .. (" then\n" .. _g651))))
    end
    if _g652 and target == "js" then
      str = str .. (" else {\n" .. (_g652 .. (ind .. "}")))
    else
      if _g652 then
        str = str .. (ind .. ("else\n" .. _g652))
  ...
  end, tr = true, foo = true, stmt = true, export = true}, ["%try"] = {special = function (form)
    local ind = indentation()
    indent_level = indent_level + 1
    local _g655 = compile(form, {_stash = true, stmt = true})
    indent_level = indent_level - 1
    local body = _g655
    local e = make_id()
    local hf = {"return", {"%array", false, {"get", e, "\"message\""}}}
    indent_level = indent_level + 1
    local _g656 = compile(hf, {_stash = true, stmt = true})
    indent_level = indent_level - 1
    local h = _g656
    return(ind .. ("try {\n" .. (body .. (ind .. ("}\n" .. (ind .. ("catch (" .. (e .. (") {\n" .. (h .. (ind .. "}\n")))))))))))
  end, tr = true, foo = true, stmt = true, export = true}, ["break"] = {stmt = true, export = true, special = function ()
  ...
  end, tr = true, foo = true, stmt = true, export = true}, ["return"] = {stmt = true, export = true, special = function (x)
    local _g717
    if nil63(x) then
      _g717 = "return"
    else
      _g717 = "return(" .. (compile(x) .. ")")
    end
    local _g657 = _g717
  ...
  end, export = true, foo = true}, ["not"] = {}, ["%local"] = {stmt = true, export = true, special = function (name, value)
    ...
    local _g722
    if target == "js" then
      _g722 = "throw new " .. compile({"Error", x})
    else
      _g722 = "error(" .. (compile(x) .. ")")
    end
    local e = _g722
    return(indentation() .. e)
  end, foo = true}, ["while"] = {special = function (cond, form)
    local _g659 = compile(cond)
    indent_level = indent_level + 1
    ...
    local ind = indentation()
    if target == "js" then
      return(ind .. ("while (" .. (_g659 .. (") {\n" .. (body .. (ind .. "}\n"))))))
    else
      return(ind .. ("while " .. (_g659 .. (" do\n" .. (body .. (ind .. "end\n"))))))
    end
  end, tr = true, foo = true, stmt = true, export = true}}, import = {"runtime", "utilities", "special", "core", "compiler"}}, lib = {export = {}, import = {"core", "special"}}, core = {export = {["define-macro"] = {macro = function (name, args, ...)
    local body = unstash({...})
    local _g661 = sub(body, 0)
    local form = join({"fn", args}, _g661)
    local _g662 = {"setenv", {"quote", name}}
    _g662.macro = form
    _g662.form = {"quote", form}
    eval(_g662)
    return(nil)
  end, export = true}, ["cat!"] = {macro = function (a, ...)
    local bs = unstash({...})
    local _g663 = sub(bs, 0)
    return({"set", a, join({"cat", a}, _g663)})
  end, export = true}, ["join!"] = {macro = function (a, ...)
    local bs = unstash({...})
    local _g664 = sub(bs, 0)
    return({"set", a, join({"join*", a}, _g664)})
    ...
    local _g666 = join({"do"}, macroexpand(_g665))
    drop(environment)
    return(_g666)
    ...
    local l = {}
    local _g668 = elements
    local _g669 = 0
    while _g669 < length(_g668) do
      local e = _g668[_g669 + 1]
      l[e] = true
      _g669 = _g669 + 1
    end
    return(join({"table"}, l))
    ...
    local _g671 = {"table"}
    _g671._scope = scope
    return({"do", {"add", "environment", _g671}, {"let", {x, join({"do"}, _g670)}, {"drop", "environment"}, x}})
  ...
  end, global = true}, fn = {macro = function (args, ...)
    local body = unstash({...})
    local _g672 = sub(body, 0)
    local _g673 = bind42(args, _g672)
    local _g674 = _g673[1]
    local _g675 = _g673[2]
    return(join({"%function", _g674}, _g675))
    ...
    map(function (_g677)
      local lh = _g677[1]
      local rh = _g677[2]
      local _g678 = bind(lh, rh)
      local _g679 = 0
      while _g679 < length(_g678) do
        local _g680 = _g678[_g679 + 1]
        local id = _g680[1]
        local val = _g680[2]
    ...
    return(join({"do"}, join(locals, {join({"let-symbol", renames}, _g676)})))
  end, export = true}, ["with-bindings"] = {macro = function (_g681, ...)
    local names = _g681[1]
    local body = unstash({...})
    ...
    _g684.variable = true
    local _g683 = {"with-frame", {"each", {x}, names, _g684}}
    _g683.scope = true
    return(join(_g683, _g682))
    ...
    local _g686 = join({"do"}, macroexpand(_g685))
    drop(environment)
    return(_g686)
    ...
    if nil63(v) then
      ...
      local i = _g724
      _g723 = {"let", {i, 0}, {"while", {"<", i, {"length", t1}}, join({"let", {k, {"at", t1, i}}}, _g687), {"inc", i}}}
    else
      local _g688 = {"target"}
      _g688.js = {"isNaN", {"parseInt", k}}
      _g688.lua = {"not", {"number?", k}}
      _g723 = {"let", {k, "nil"}, {"%for", t1, k, {"when", _g688, join({"let", {v, {"get", t1, k}}}, _g687)}}}
    end
    return({"let", {t1, t}, _g723})
  end, export = true}, pr = {macro = function (...)
    local xs = unstash({...})
    local _g689 = map(function (x)
      return(splice({{"to-string", x}, "\" \""}))
    end, xs)
    return({"print", join({"cat"}, _g689)})
    ...
    local imports = {}
    local exp = _g692.export
    local imp = _g692.import
    local _g693 = imp or {}
    local _g694 = 0
    ...
    if some63(_g697) then
      local _g698 = bind42(x, _g697)
      local args = _g698[1]
      local _g699 = _g698[2]
      return(join({"%local-function", name, args}, _g699))
    else
      return({"%local", name, x})
    end
    ...
    if some63(_g700) then
      local _g701 = bind42(x, _g700)
      local args = _g701[1]
      local _g702 = _g701[2]
      return(join({"%global-function", name, args}, _g702))
    else
      if target == "js" then
    ...
    local body = unstash({...})
    return(join({"%object"}, mapo(function (_g351, x)
      return(x)
    end, body)))
    ...
    return({"if", {"not", cond}, join({"do"}, _g706)})
  end, export = true}}, import = {"runtime", "utilities", "special", "core", "compiler"}}, reader = {export = {["define-reader"] = {macro = function (_g707, ...)
    local char = _g707[1]
    local stream = _g707[2]
    local body = unstash({...})
    local _g708 = sub(body, 0)
    return({"set", {"get", "read-table", char}, join({"fn", {stream}}, _g708)})
  end, export = true}, ["flag?"] = {variable = true}, ["read-char"] = {variable = true}, ["key?"] = {variable = true}, ["read-table"] = {variable = true, export = true}, eof = {variable = true}, ["skip-non-code"] = {variable = true}, delimiters = {variable = true}, ["peek-char"] = {variable = true}, whitespace = {variable = true}, read = {variable = true, export = true}, ["read-all"] = {variable = true, export = true}, ["read-from-string"] = {variable = true, export = true}, ["make-stream"] = {variable = true, export = true}}, import = {"runtime", "special", "core"}}, boot = {export = {["%initial-modules"] = {macro = function ()
    return(quote_modules())
  end}, ["%initial-environment"] = {macro = function ()
    return(quote_environment(initial_environment()))
  end}, modules = {global = true, export = true}}, import = {"runtime", "utilities", "special", "core", "compiler"}}, optimizer = {export = {optimizations = {variable = true}, optimize = {variable = true, export = true}, ["define-optimization"] = {}}, import = {"runtime", "special", "core"}}, main = {export = {save = {macro = function (...)
    local specs = unstash({...})
    local _g709 = sub(specs, 0)
    map(compile_module, _g709)
    return(nil)
  end}}, import = {"runtime", "special", "core", "reader", "compiler"}}, system = {export = {nexus = {global = true, export = true}}, import = {"special", "core"}}, runtime = {export = {replicate = {variable = true, export = true}, extend = {variable = true, export = true}, ["some?"] = {variable = true, export = true}, ["to-string"] = {variable = true, export = true}, [">"] = {variable = true, export = true}, ["="] = {variable = true, export = true}, ["<"] = {variable = true, export = true}, ["parse-number"] = {variable = true, export = true}, iterate = {variable = true, export = true}, split = {variable = true, export = true}, exclude = {variable = true, export = true}, sub = {variable = true, export = true}, substring = {variable = true, export = true}, splice = {variable = true, export = true}, ["composite?"] = {variable = true, export = true}, reduce = {variable = true, export = true}, ["table?"] = {variable = true, export = true}, reverse = {variable = true, export = true}, apply = {variable = true, export = true}, join = {variable = true, export = true}, module = {variable = true, export = true}, code = {variable = true, export = true}, ["boolean?"] = {variable = true, export = true}, ["-"] = {variable = true, export = true}, inner = {variable = true, export = true}, ["*"] = {variable = true, export = true}, length = {variable = true, export = true}, ["%"] = {variable = true, export = true}, ["empty?"] = {variable = true, export = true}, search = {variable = true, export = true}, pairwise = {variable = true, export = true}, keep = {variable = true, export = true}, ["is?"] = {variable = true, export = true}, map = {variable = true, export = true}, write = {variable = true, export = true}, ["<="] = {variable = true, export = true}, [">="] = {variable = true, export = true}, ["id-literal?"] = {variable = true, export = true}, ["write-file"] = {variable = true, export = true}, ["%message-handler"] = {variable = true, export = true}, ["string?"] = {variable = true, export = true}, ["+"] = {variable = true, export = true}, find = {variable = true, export = true}, mapl = {variable = true}, sublist = {variable = true, export = true}, ["string-literal?"] = {variable = true, export = true}, ["keys?"] = {variable = true, export = true}, ["splice?"] = {variable = true}, setenv = {variable = true, export = true}, ["read-file"] = {variable = true, export = true}, add = {variable = true, export = true}, ["toplevel?"] = {variable = true, export = true}, stash = {variable = true, export = true}, ["make-id"] = {variable = true, export = true}, ["none?"] = {variable = true, export = true}, ["atom?"] = {variable = true, export = true}, exit = {variable = true, export = true}, tl = {variable = true, export = true}, ["module-key"] = {variable = true, export = true}, ["nil?"] = {variable = true, export = true}, ["number?"] = {variable = true, export = true}, ["/"] = {variable = true, export = true}, ["id-count"] = {variable = true}, cat = {variable = true, export = true}, unstash = {variable = true, export = true}, last = {variable = true, export = true}, ["list?"] = {variable = true, export = true}, hd = {variable = true, export = true}, char = {variable = true, export = true}, drop = {variable = true, export = true}, ["function?"] = {variable = true, export = true}, ["in?"] = {variable = true, export = true}}, import = {"special", "core"}}, compiler = {export = {["load-module"] = {variable = true, export = true}, compile = {variable = true, export = true}, ["lower-statement"] = {variable = true}, ["lower-infix?"] = {variable = true}, ["lower-for"] = {variable = true}, ["compile-args"] = {variable = true}, ["lower-infix"] = {variable = true}, ["lower-function"] = {variable = true}, infix = {variable = true}, ["op-delims"] = {variable = true}, ["lower-body"] = {variable = true}, ["%compile-module"] = {variable = true}, ["compile-function"] = {variable = true, export = true}, ["infix?"] = {variable = true}, ["in-module"] = {variable = true, export = true}, ["unary?"] = {variable = true}, ["compile-infix"] = {variable = true}, terminator = {variable = true}, ["lower-try"] = {variable = true}, ["can-return?"] = {variable = true}, ["lower-while"] = {variable = true}, ["compiler-output"] = {variable = true}, ["compile-call"] = {variable = true}, run = {variable = true}, process = {variable = true}, eval = {variable = true, export = true}, ["lower-definition"] = {variable = true}, getop = {variable = true}, lower = {export = true, global = true, variable = true}, precedence = {variable = true}, ["compile-module"] = {variable = true, export = true}, prologue = {variable = true}, ["compiling?"] = {variable = true}, ["%result"] = {global = true, export = true}, ["compile-file"] = {variable = true}, ["compile-special"] = {variable = true}, encapsulate = {variable = true}, ["module-path"] = {variable = true}, ["current-module"] = {global = true, export = true}, ["lower-short"] = {variable = true}, ["open-module"] = {variable = true, export = true}, ["lower-if"] = {variable = true}, ["lower-special"] = {variable = true}, ["compile-atom"] = {variable = true}, ["lower-call"] = {variable = true}, ["lower-do"] = {variable = true}}, import = {"runtime", "utilities", "special", "core", "reader"}}, utilities = {export = {["can-unquote?"] = {variable = true}, ["special-form?"] = {variable = true, export = true}, ["toplevel?"] = {variable = true, export = true}, ["variable?"] = {variable = true, export = true}, reserved = {variable = true}, mapo = {variable = true, export = true}, ["quote-frame"] = {variable = true}, ["global?"] = {variable = true}, ["indent-level"] = {global = true, export = true}, imported = {variable = true, export = true}, quoted = {variable = true, export = true}, ["bound?"] = {variable = true, export = true}, ["statement?"] = {variable = true, export = true}, ["quoting?"] = {variable = true}, ["quote-module"] = {variable = true}, escape = {variable = true}, ["bind*"] = {variable = true, export = true}, quasiexpand = {variable = true, export = true}, ["valid-char?"] = {variable = true}, ["symbol?"] = {variable = true, export = true}, ["with-indent"] = {macro = function (form)
    local result = make_id()
    return({"do", {"inc", "indent-level"}, {"let", {result, form}, {"dec", "indent-level"}, result}})
  end, export = true}, exported = {variable = true, export = true}, getenv = {variable = true, export = true}, ["quote-binding"] = {variable = true}, ["stash*"] = {variable = true, export = true}, ["numeric?"] = {variable = true}, ["quasisplice?"] = {variable = true}, ["initial-environment"] = {variable = true, export = true}, ["quote-modules"] = {variable = true, export = true}, ["quote-environment"] = {variable = true, export = true}, ["special?"] = {variable = true, export = true}, ["quasiquoting?"] = {variable = true}, ["reserved?"] = {variable = true, export = true}, ["symbol-expansion"] = {variable = true, export = true}, ["quasiquote-list"] = {variable = true}, ["macro?"] = {variable = true, export = true}, indentation = {variable = true, export = true}, ["valid-id?"] = {variable = true, export = true}, ["macro-function"] = {variable = true, export = true}, ["to-id"] = {variable = true, export = true}, bind = {variable = true, export = true}, macroexpand = {variable = true, export = true}}, import = {"runtime", "special", "core"}}}
