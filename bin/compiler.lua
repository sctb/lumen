...
function macroexpand(form)
  if symbol63(form) then
    ...
    if atom63(form) then
      ...
      local __x53 = hd(form)
      if __x53 == "%expansion" then
        return form[2]
      else
        if __x53 == "%local" then
          return expand_local(form)
        else
          if __x53 == "%function" then
            return expand_function(form)
          else
            if __x53 == "%global-function" then
              return expand_definition(form)
            else
              if __x53 == "%local-function" then
                return expand_definition(form)
              else
                if macro63(__x53) then
