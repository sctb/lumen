...
function bind(lh, rh)
  if atom63(lh) or vector63(lh) then
    return({lh, rh})
  else
    ...
    for __k1 in next, ____o1 do
      ...
      if __k1 == "rest" then
        ...
        __e24 = {"get", __id, {"quote", bias(__k1)}}
      end
      local __x5 = __e24
      if is63(__k1) then
        local __e25
...
local function expand_local(__x38)
  ...
  if vector63(__name) then
    local ____x40 = tl(__name)
    local ____i5 = 0
    while ____i5 < _35(____x40) do
      local __x41 = ____x40[____i5 + 1]
...
local function quasiquote_list(form, depth)
  ...
  local ____x57 = form
  local ____i9 = 0
  while ____i9 < _35(____x57) do
    local __x58 = ____x57[____i9 + 1]
...
local function compile_args(args)
  ...
  local ____x85 = args
  local ____i16 = 0
  while ____i16 < _35(____x85) do
    local __x86 = ____x85[____i16 + 1]
...
local function lower_do(args, hoist, stmt63, tail63)
  ...
  local ____i18 = 0
  while ____i18 < _35(____x100) do
    local __x101 = ____x100[____i18 + 1]
...
function immediate_call63(x)
  return(obj63(x) and hd63(hd(x), "%function"))
end
setenv("do", {_stash = true, special = function (...)
  ...
  local ____i20 = 0
  while ____i20 < _35(____x142) do
    local __x143 = ____x142[____i20 + 1]
...
setenv("%try", {_stash = true, special = function (form)
  ...
  local __hf1 = {"return", {"%array", false, __e9}}
  indent_level = indent_level + 1
...
function compile_binding(form)
  if vector63(form) then
    local __s6 = inner(compile_args(tl(form)))
    if target == "js" then
      return("[" .. __s6 .. "]")
    else
      return(__s6)
...
setenv("return", {_stash = true, special = function (x)
  ...
  if nil63(x) then
    ...
    local __e49
    if vector63(x) then
      __e49 = "return " .. compile_binding(x)
    else
...
setenv("%local", {_stash = true, special = function (name, value)
  local __id28 = compile_binding(name)
