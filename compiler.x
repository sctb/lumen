...
(defun compile-list (forms quoted? quasi?)
  (local open (if (= current-target 'lua) "{" "["))
  (local close (if (= current-target 'lua) "}" "]"))
  (local str "")
  (across (forms x i)
    (if (and quasi? (list? x) (= (at x 0) 'unquote-splicing))
	(do (local x1 (compile (at x 1)))
	    (local x2 (compile-list (sub forms (+ i 1)) true quasi?))
	    (set open (cat "join(" open))
	    (set close (cat close ",join(" x1 "," x2 "))"))
	    break)
      (do (local x1 (if quoted? (quote-form x quasi?) (compile x)))
	  (set str (cat str x1))
	  (if (< i (- (length forms) 1)) (set str (cat str ","))))))
  (cat open str close))
...
(defun quote-form (form quasi?)
  (if (atom? form)
	...
	(to-string form))
    (and quasi? (= (at form 0) 'unquote)) (compile (at form 1))
    (compile-list form true quasi?)))

(defun compile-quote (forms)
  (quote-form (at forms 0)))

(defun compile-quasiquote (forms)
  (quote-form (at forms 0) true))
