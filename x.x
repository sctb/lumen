...
(function compile-special (form stmt?)
  (local name (at form 0))
  (local delim? (and stmt? (not (get self-terminating name))))
  (local delim (? delim? ";" ""))
  (local fn (get special name))
  (return (cat (fn (sub form 1)) delim)))
...
(function compile (form stmt?)
  (local delim (? stmt? ";" ""))
  (if ((= form nil) (return ""))
      ((atom? form) (return (cat (compile-atom form) delim)))
      ((call? form)
       (if ((operator? form)
            (return (cat (compile-operator form) delim)))
           ((special? form)
