...
(defun usage ()
  (print "usage: x [<inputs>] [-i] [-o <output>] [-l <language>] [-e <exp>]")
  (exit))
...
(do (local inputs '())
    ...
    (local exp false)
    (across (args arg i)
      (if (or (= arg "-o") (= arg "-l") (= arg "-i") (= arg "-e"))
	  (if (or (= arg "-i") (> (length args) (+ i 1)))
	      (do (set i (+ i 1))
	  ...
	  (= "-" (sub arg 0 1)) (print "unrecognized option:" arg)
	(push inputs arg)))
    (if (and (not output)
	     (> (length inputs) 0))
	(do (local input (at inputs 0))
	    (local name (sub input 0 (find input ".")))
	    (set output (cat name "." current-target))))
    (local compiled (compile-files inputs))
    (if interactive? (interactive)
        exp (print (to-string (eval-string exp)))
        output (write-file output compiled)))
